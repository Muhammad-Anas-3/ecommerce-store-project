...
export default async function QueryProducts({ params }: { params: { slug: string } }) {

    const products = await db.product.findMany({ where: { category: params.slug } })
    return (
            ...
            <div className="flex justify-between items-end container my-5">
                <div>
                    ...
                    <h2 className="text-gray-500 mt-2 tracking-wide text-base font-medium sm:text-xl text-center sm:text-start">
                        Explore  {params.slug} we offer from around the world
                    </h2>
                </div>
            </div>
            <Suspense
